...
```{r some Basic cleaning, include=FALSE}
...
```{r gender representation in comics, include=FALSE}
summary(FullComics)
...
##Character Alignment - Factors that influence this
...
##The type of identity with Years

```{r ID with Age}

IDs <- aggregate(ALIGN ~ ID + YEAR + Comics, data = FullComics, FUN = length)
#table(IDs$ID)
ggplot(data = IDs, aes(YEAR, ALIGN, col = ID)) + geom_freqpoly(stat = "identity", lwd = 1) + facet_grid(Comics~.)
#ggplotly()
length(FullComics$ID[FullComics$ID == "Public Identity"])
...
```{r prominent charachters, include=FALSE}
...
```{r likable features, include=FALSE}
...
```{r difference of appearance in good and bad characters in DC, echo=FALSE, fig.width=12, fig.height=10}
...
allign_model <- rpart(ALIGN ~ EYE + HAIR + SEX + ID + APPEARANCES + YEAR + ALIVE, data = trainsetDC, control = rpart.control(cp = 0.01))
#require(fancyRpartPlot)
...
```{r difference of appearance in good and bad characters in Marvel, echo=FALSE, fig.width=12, fig.height=10}
allignment <- subset(FullComics, FullComics$ALIGN %in% c("Good Characters", "Bad Characters"))
allignmentMArvel <- subset(allignment, allignment$Comics == "Marvel")
...
#creating a decision tree
allign_model_marvel <- rpart(ALIGN ~ EYE + HAIR + SEX + ID + APPEARANCES + YEAR, data = trainsetMarvel, control = rpart.control(cp = 0.006))
#summary(allign_model_marvel)
fancyRpartPlot(allign_model_marvel)
##checking the accuracy of the model
predictMarvel <- predict(allign_model_marvel, newdata = testsetMarvel, type = "class")
...
View(allign_model_marvel$cptable)
#options(repos="https://cran.rstudio.com" )
...
```{r random forests, eval=FALSE, include=FALSE}

summary(trainsetMarvel)

MarvelRF <- read.csv("Marvel.csv", na.strings = '', stringsAsFactors = TRUE)
summary(MarvelRF)
##sampling in 70:30 ratio

indexRF <- nrow(MarvelRF)
trainIndexRF <- sample(indexRF, 0.7*indexRF)
trainRFMarvel <- MarvelRF[trainIndexRF,]
testRFMarvel <- MarvelRF[-trainIndexRF,]


ComicsRF <- randomForest(ALIGN ~ EYE + HAIR + SEX + GSM + ALIVE + APPEARANCES + Year , data = trainRFMarvel,na.action = rfImpute, importance = TRUE, ntree = 1200, proximity = TRUE)
predictedRF <- predict(ComicsRF, newdata = testRFMarvel)

library(caret)
confusionMatrix(predictedRF, testRFMarvel$ALIGN)
